...
import com.msg.annotation.Errors;
...
public class CommonExceptionHandler{
    ...
    public ModelMap defaultErrorHandler(HttpServletRequest req, Exception e) throws Exception {
		...
		if(e.getCause() instanceof MySQLIntegrityConstraintViolationException){
			...
			if(e2.getErrorCode() == MsgConfig.PK_UK_ERROR_CODE){
		...
		for (Field field : fields) {
			if (field.isAnnotationPresent(Errors.class)) {
				...
				if (annotation != null && annotation.code() == code) {
					if(env != null && !StringUtils.isEmpty(env.getProperty(MsgConfig.CONFIG_KEY_ERROR)) && env.getProperty(MsgConfig.CONFIG_KEY_ERROR).equals(MsgConfig.CONFIG_ERROR_VALUE_EN)){
