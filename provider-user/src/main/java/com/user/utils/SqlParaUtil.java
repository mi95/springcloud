...
//        initParaList();
    ...
    public SqlParaUtil(String sql, Object... params) {
        ininSql();
        if (CommonException.verifyObjectsB(params)) {
            initParaList();
        }
        addSql(sql);
    ...
    public SqlParaUtil andColsLikeTo(String key, String... names) {
        if (CommonException.verifyObjectsB(key, names)) {
            this.sql.append(" and (");
            key = ("%" + key + "%");
            for (String name : names) {
                if (!CommonException.verifyObjectsB(name)) {
                    continue;
                }
    ...
    public SqlParaUtil andColEqualsTo(Object param, String name) {
        if (CommonException.verifyObjectsB(param, name)) {
    ...
    public void deleteCharAt(Integer index) {
        CommonException.verifyObjects(index);
