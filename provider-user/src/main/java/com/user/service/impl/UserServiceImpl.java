...
public class UserServiceImpl implements UserService {
    ...
    public void addUser(UserInfo userInfo) {
        CommonException.verifyObjects(userInfo);
        CommonException.opeResult(userInfo.save());
    }

    @Override
    public void editUser(UserInfo userInfo) {
        CommonException.verifyObjects(userInfo, userInfo.getId());
        CommonException.opeResult(userInfo.update());
    }

    @Override
    public void delUser(Long userId) {
        CommonException.verifyObjects(userId);
        CommonException.opeResult(UserInfo.dao.deleteById(userId));
    }

    @Override
    public UserInfo findUser(UserInfo user) {
        CommonException.verifyObjects(user);
    ...
    public Page<Record> findUserPageList(Integer pageNum, Integer pageSize, UserInfo userInfo) {
        ...
        if (CommonException.verifyObjectsB(userInfo.getId())) {
        ...
        if (CommonException.verifyIntegerB(userInfo.getState())) {
